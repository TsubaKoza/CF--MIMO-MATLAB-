...
%GENERATERAYTRACINGCHANNELS3D Ground-truth LoS + single-bounce channels.
% h_true is known only to the simulator. It is never passed to an AP-side
% estimator except through the simulated received pilot matrix.
...
for m = 1:M
    ...
    for k = 1:K
        ...
        paths = [paths(:); reflected(:); diffracted(:)]; %#ok<AGROW>
        for l = 1:numel(paths)
            h_true(:,m,k) = h_true(:,m,k)+paths(l).contribution;
        end
        pathInfo(m,k).hasLoS = hasLoS;
        pathInfo(m,k).blockingObstacleIndices = blocking;
        pathInfo(m,k).numReflections = numel(reflected);
        pathInfo(m,k).numDiffractions = numel(diffracted);
